...
public:
    ...
    CBigNum(signed char n)        { pbn = BN_new(); if (pbn == NULL) throw bignum_error("CBigNum::CBigNum() : BN_new failed"); if (n >= 0) setulong(n); else setint64(n); }
    CBigNum(short n)              { pbn = BN_new(); if (pbn == NULL) throw bignum_error("CBigNum::CBigNum() : BN_new failed"); if (n >= 0) setulong(n); else setint64(n); }
    CBigNum(int n)                { pbn = BN_new(); if (pbn == NULL) throw bignum_error("CBigNum::CBigNum() : BN_new failed"); if (n >= 0) setulong(n); else setint64(n); }
    CBigNum(long n)               { pbn = BN_new(); if (pbn == NULL) throw bignum_error("CBigNum::CBigNum() : BN_new failed"); if (n >= 0) setulong(n); else setint64(n); }
    CBigNum(long long n)          { pbn = BN_new(); if (pbn == NULL) throw bignum_error("CBigNum::CBigNum() : BN_new failed"); setint64(n); }
    CBigNum(unsigned char n)      { pbn = BN_new(); if (pbn == NULL) throw bignum_error("CBigNum::CBigNum() : BN_new failed"); setulong(n); }
    CBigNum(unsigned short n)     { pbn = BN_new(); if (pbn == NULL) throw bignum_error("CBigNum::CBigNum() : BN_new failed"); setulong(n); }
    CBigNum(unsigned int n)       { pbn = BN_new(); if (pbn == NULL) throw bignum_error("CBigNum::CBigNum() : BN_new failed"); setulong(n); }
    CBigNum(unsigned long n)      { pbn = BN_new(); if (pbn == NULL) throw bignum_error("CBigNum::CBigNum() : BN_new failed"); setulong(n); }
    CBigNum(unsigned long long n) { pbn = BN_new(); if (pbn == NULL) throw bignum_error("CBigNum::CBigNum() : BN_new failed"); setuint64(n); }
    explicit CBigNum(uint256 n)   { pbn = BN_new(); if (pbn == NULL) throw bignum_error("CBigNum::CBigNum() : BN_new failed"); setuint256(n); }

    explicit CBigNum(const std::vector<unsigned char>& vch) : pbn(BN_new())
    {
        if (pbn == NULL)
            throw bignum_error("CBigNum::CBigNum() : BN_new failed");
        setvch(vch);
    ...
    */
    bool isPrime(const int checks=BN_prime_checks) const {
        CAutoBN_CTX pctx;
        int ret = BN_is_prime(pbn, checks, NULL, pctx, NULL);
        if(ret < 0){
    ...
    BIGNUM* getBN() { return pbn; }
    const BIGNUM* getBN() const { return pbn; }
...
inline const CBigNum operator%(const CBigNum& a, const CBigNum& b)
{
    CAutoBN_CTX pctx;
    CBigNum r;
    if (!BN_nnmod(r.getBN(), a.getBN(), b.getBN(), pctx))
        throw bignum_error("CBigNum::operator% : BN_div failed");
    return r;
}
